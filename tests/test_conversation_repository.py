...
class TestConversationRepository(unittest.TestCase):
    ...
    def test_save_speaker_and_speech_content_list_success(self):
        ...
        with patch.object(self.repository, '_save_conversation', side_effect=mock_conversation_ids):
            # 実行
    ...
    def test_save_speaker_and_speech_content_list_database_error(self):
        ...
        with patch.object(self.repository, '_save_conversation', side_effect=Exception("DB Error")):
            # エラーが発生することを検証
    ...
    def test_get_all_conversations(self):
        ...
        # モックデータ準備
        mock_conversations = [
            (1, None, 123, "委員長(田中太郎)", "議事を開始します。", 1, 1, 1, "2024-01-01", "2024-01-01", "田中太郎"),
            (2, None, None, "◆委員(佐藤花子)", "質問があります。", 2, 1, 2, "2024-01-01", "2024-01-01", None)
        ]
        
        mock_result = MagicMock()
        mock_result.fetchall.return_value = mock_conversations
        mock_result.keys.return_value = [
            'id', 'minutes_id', 'speaker_id', 'speaker_name', 'comment',
            'sequence_number', 'chapter_number', 'sub_chapter_number',
            'created_at', 'updated_at', 'linked_speaker_name'
        ]
        self.mock_session.execute.return_value = mock_result
    ...
    def test_get_speaker_linking_stats(self):
        """発言者紐付け統計取得テスト"""
        mock_result = MagicMock()
        mock_result.fetchone.return_value = [100, 75, 25]  # 総数, 紐付けあり, 紐付けなし
        self.mock_session.execute.return_value = mock_result
