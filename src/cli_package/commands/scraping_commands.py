...
class ScrapingCommands(BaseCommand):
    ...
    def scrape_minutes(url, output_dir, format, no_cache, upload_to_gcs, gcs_bucket):
        ...
        success = asyncio.run(ScrapingCommands._async_scrape_minutes(
    ...
    @staticmethod
    async def _async_scrape_minutes(url, output_dir, format, no_cache, upload_to_gcs, gcs_bucket):
        ...
        ScrapingCommands.show_progress(f"Date: {minutes.date.strftime('%Y年%m月%d日') if minutes.date else 'Unknown'}")
        ScrapingCommands.show_progress(f"Speakers found: {len(minutes.speakers)}")
        ScrapingCommands.show_progress(f"Content length: {len(minutes.content)} characters")
        if minutes.pdf_url:
    ...
    def batch_scrape(tenant, start_id, end_id, max_schedule, output_dir, concurrent, upload_to_gcs, gcs_bucket):
        ...
        """
        ScrapingCommands.show_progress(f"Batch scraping from kaigiroku.net tenant: {tenant}")
        ...
        success_count = asyncio.run(ScrapingCommands._async_batch_scrape(
            urls, output_dir, concurrent, upload_to_gcs, gcs_bucket
        ))
        
        ScrapingCommands.success(f"Saved {success_count} meeting minutes to {output_dir}")
    
    @staticmethod
    async def _async_batch_scrape(urls, output_dir, concurrent, upload_to_gcs, gcs_bucket):
            ...
            if minutes:
                ...
                if txt_success and json_success:
                    ...
                    if txt_gcs_url or json_gcs_url:
                        ScrapingCommands.show_progress(f"Scraped and uploaded: {base_name}")
        
        ScrapingCommands.show_progress(f"\nCompleted: {success_count}/{len(urls)} URLs successfully scraped")
