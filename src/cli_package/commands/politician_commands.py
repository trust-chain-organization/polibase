...
class PoliticianCommands(BaseCommand):
    ...
    def scrape_politicians(party_id, all_parties, dry_run, max_pages):
        ...
        asyncio.run(PoliticianCommands._async_scrape_politicians(
    ...
    async def _async_scrape_politicians(party_id, all_parties, dry_run, max_pages):
        ...
        try:
            ...
            if not parties:
                PoliticianCommands.error("No parties found with member list URLs", exit_code=0)
                return
            
            PoliticianCommands.show_progress(f"Found {len(parties)} parties to scrape:")
            for party in parties:
                PoliticianCommands.show_progress(f"  - {party.name}: {party.members_list_url}")
