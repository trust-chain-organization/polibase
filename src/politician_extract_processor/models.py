...
class PoliticianInfo(BaseModel):
    ...
    speech_count: int = Field(default=0, description="発言回数")
    speech_contents: List[str] = Field(default_factory=list, description="発言内容のリスト")
...
class PoliticianProcessState(BaseModel):
    ...
    processed_minutes: str = Field(default="", description="前処理を施した議事録")
