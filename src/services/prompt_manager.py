...
抽出ルール:
1. 実際に発言した人の発言内容のみを抽出してください
2. 以下は発言として扱わないでください:
   - 出席者リスト（「出席委員」「欠席委員」などの単なる名前の羅列）
   - 配付資料リスト
   - 会議の基本情報（日時、場所、出席者など）
   - セクションのタイトルや見出しのみ
   - 議事録末尾の署名（例：「委員長　○○」のような単独の名前）
...
6. 実際の発言は通常「○発言者名」の後に続く文章として記載されています
7. 名前だけが単独で記載されている場合（署名など）は発言として扱わないでください

各発言について以下の情報を含めてください：
- speech_order: 発言順序（1から始まる連番）
- speaker: 発言者名（人名のみ）
- speech_content: 発言内容（実際に話された内容）
