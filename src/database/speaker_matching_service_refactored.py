...
class SpeakerMatch(BaseModel):
    ...
    speaker_id: Optional[int] = Field(description="マッチしたspeakerのID", default=None)
    speaker_name: Optional[str] = Field(description="マッチしたspeakerの名前", default=None)
...
class SpeakerMatchingService:
    ...
    def __init__(self, llm: Optional[ChatGoogleGenerativeAI] = None):
        ...
        self._matching_chain = self.chain_factory.create_speaker_matching_chain(SpeakerMatch)
    ...
    def find_best_match(self, speaker_name: str) -> SpeakerMatch:
        ...
        try:
            ...
            filtered_speakers = self._filter_candidates(speaker_name, available_speakers)
    ...
    def _rule_based_matching(self, speaker_name: str, available_speakers: List[Dict]) -> SpeakerMatch:
        """従来のルールベースマッチング"""
    ...
    def _filter_candidates(self, speaker_name: str, available_speakers: List[Dict], max_candidates: int = 10) -> List[Dict]:
        ...
        for speaker in available_speakers:
            ...
            # 括弧内名前との一致
            if extracted_name and (speaker['name'] == extracted_name or extracted_name in speaker['name']):
                score += 5
        ...
        # 最大候補数に制限
        return candidates[:max_candidates] if candidates else available_speakers[:max_candidates]
        ...
        try:
            ...
            for conversation_id, speaker_name in unlinked_conversations:
                ...
                if match_result.matched and match_result.speaker_id:
                    ...
                    logger.info(f"  {confidence_emoji} マッチ成功: {speaker_name} → {match_result.speaker_name} (信頼度: {match_result.confidence:.2f})")
                else:
                    stats['failed_matches'] += 1
                    logger.warning(f"  🔴 マッチ失敗: {speaker_name} ({match_result.reason})")
