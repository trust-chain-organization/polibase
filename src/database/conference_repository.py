...
class ConferenceRepository:
    ...
    def get_conference_by_id(self, conference_id: int) -> Optional[Dict]:
        """IDで会議体を取得"""
        if not self.connection:
            self.connection = self.engine.connect()
    ...
    def get_conferences_by_governing_body(self, governing_body_id: int) -> List[Dict]:
        ...
        result = self.connection.execute(query, {"governing_body_id": governing_body_id})
        conferences = []
        for row in result:
            conferences.append({
                'id': row.id,
    ...
    def create_conference(self, name: str, governing_body_id: int, type: Optional[str] = None) -> Optional[int]:
        """新しい会議体を作成"""
        if not self.connection:
            self.connection = self.engine.connect()
    ...
    def update_conference(self, conference_id: int, name: str, type: Optional[str] = None) -> bool:
        """会議体を更新"""
        if not self.connection:
            self.connection = self.engine.connect()
    ...
    def delete_conference(self, conference_id: int) -> bool:
        ...
        try:
            ...
            result = self.connection.execute(check_query, {"conference_id": conference_id})
            count = result.fetchone().count
            
            if count > 0:
                logger.warning(f"Cannot delete conference {conference_id}: has {count} related meetings")
                return False
