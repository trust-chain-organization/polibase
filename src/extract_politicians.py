...
def save_to_database(politician_info_list_obj: PoliticianInfoList) -> List[int]:
    ...
    """
    if not politician_info_list_obj or not politician_info_list_obj.politician_info_list:
    ...
    try:
        ...
        raise DatabaseError(
            ...
            {"count": len(politician_info_list_obj.politician_info_list), "error": str(e)}
        )
...
def display_database_status() -> None:
    ...
    try:
        ...
        raise DatabaseError(
            "Failed to retrieve speaker database status",
            {"error": str(e)}
        )
