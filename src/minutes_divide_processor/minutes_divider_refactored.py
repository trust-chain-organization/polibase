...
class MinutesDivider:
    ...
    def pre_process(self, original_minutes: str) -> str:
        ...
        processed_minutes = re.sub(r'[ ]+', ' ', processed_minutes).strip()  # 連続するスペースを1つのスペースに置換
    ...
    def do_divide(self, processed_minutes: str, section_info_list: SectionInfoList) -> SectionStringList:
        ...
        while i < len(section_info_list_data):
            ...
            if i == 0 and start_index == 0:
                ...
                if start_index == -1:
                    start_index = 0
                    logger.warning(f"最初のキーワード '{keyword}' が見つからないため、議事録の先頭から開始します")
            
            # キーワードが見つからない場合はスキップ
            if start_index == -1:
                logger.warning(f"キーワード '{keyword}' が議事録に見つかりません。スキップします。")
            ...
            while j < len(section_info_list_data):
                ...
                if next_keyword_index != -1:
                    ...
                    logger.warning(f"キーワード '{next_keyword}' が議事録に見つかりません。スキップします。")
        ...
        try:
            ...
            for section_info in result.section_info_list:
                if section_info.chapter_number != last_chapter_number + 1:
                    logger.warning("section_infoのchapter_numberが連番になっていません。")
    ...
    def check_length(self, section_string_list: SectionStringList) -> RedivideSectionStringList:
        ...
        for index, section_string in enumerate(section_string_list.section_string_list):
            ...
            if size_in_bytes > 6000:  # 6000文字より多いか確認
                logger.info(f"section_stringの文字数が6000文字を超えています。再分割します。")
    ...
    def do_redivide(self, redivide_section_string_list: RedivideSectionStringList) -> RedividedSectionInfoList:
        ...
        redivide_chain = (
            {"minutes": RunnablePassthrough(), "original_index": RunnablePassthrough(), "divide_counter": RunnablePassthrough()} |
            prompt_template |
            self.llm_service.get_structured_llm(SectionInfoList)
        )

        section_string_list = []
        for redivide_section_string in redivide_section_string_list.redivide_section_string_list:
            ...
            result = self.chain_factory.invoke_with_retry(
                redivide_chain,
                {
                    "minutes": redivide_section_string.redivide_section_string.section_string,
        ...
        try:
            ...
            for speaker_and_speech_content in speaker_and_speech_content_list:
                if speaker_and_speech_content.speech_order != last_speech_order + 1:
                    logger.warning("speaker_and_speech_contentのspeech_orderが連番になっていません。")
