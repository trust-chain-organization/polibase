...
class SpeakerExtractor:
    ...
    SPEAKER_PATTERNS = [
        # 【山田太郎議員】
        (re.compile(r'【([^】]+(?:議員|委員|市長|町長|村長|知事|局長|部長|課長|理事|次長|参事|主幹))】'), True),
        (re.compile(r'【([^】]+)】'), False),  # 役職なしパターン
        
        # ○山田太郎議員
        (re.compile(r'○\s*([^（\s]+(?:議員|委員|市長|町長|村長|知事|局長|部長|課長|理事|次長|参事|主幹))'), True),
        (re.compile(r'○\s*([^（\s：]+)'), False),
        
        # 山田太郎議員：
        (re.compile(r'^([^：\s]+(?:議員|委員|市長|町長|村長|知事|局長|部長|課長|理事|次長|参事|主幹))：'), True),
        ...
        # 山田太郎議員 （改行や空白の後）
        (re.compile(r'^\s*([^（\s]+(?:議員|委員|市長|町長|村長|知事|局長|部長|課長|理事|次長|参事|主幹))\s*$'), True),
    ]
    
    # 役職パターン
    ROLE_PATTERNS = [
        '議員', '委員', '市長', '町長', '村長', '知事',
        '局長', '部長', '課長', '理事', '次長', '参事', '主幹',
        '議長', '副議長', '委員長', '副委員長'
    ]
    ...
    def _detect_speaker(self, line: str) -> Optional[Dict[str, str]]:
        ...
        Returns:
            {'name': '発言者名', 'role': '役職', 'remaining_text': '残りのテキスト'} or None
        """
    ...
    def _extract_from_block(self, block: BeautifulSoup) -> Optional[SpeakerData]:
    ...
    def merge_continuous_speeches(self, speakers: List[SpeakerData]) -> List[SpeakerData]:
        """同じ発言者の連続した発言をマージ
