...
class DateParser:
    ...
    def _create_datetime(self, year: int, month: int, day: int) -> Optional[datetime]:
        """datetime オブジェクトを作成"""
        try:
            return datetime(year, month, day)
        except ValueError as e:
            self.logger.warning(f"Invalid date values: year={year}, month={month}, day={day}. Error: {e}")
            return None
    
    def extract_from_text(self, text: str) -> Optional[datetime]:
        """テキストから最初に見つかった日付を抽出
