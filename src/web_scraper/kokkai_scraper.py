...
class KokkaiScraper(BaseScraper):
    ...
    async def _load_page_with_retry(
        ...
        for attempt in range(retry_count):
            try:
                logger.info(f"Loading page: {url} (attempt {attempt + 1}/{retry_count})")
    ...
    async def _extract_minutes_data(
                ...
                for row in rows:
                    ...
                    if len(cells) >= 2:
                        ...
                        if content_cell:
                            ...
                            if content_text and content_text.strip() and len(content_text.strip()) > 20:
            ...
            if not content_parts:
                ...
                if body_text:
                    ...
                    for line in lines:
                        ...
                        if len(line) > 50 and not any(skip in line for skip in ["シンプル表示", "ヘルプ", "検索", "ダウンロード"]):
                ...
                for div in speaker_divs:
                    ...
                    if text and text.strip():
                        ...
                        if len(parts) >= 2:
                            ...
                            name_parts = []
                            for part in parts:
                                if not part[0].isdigit() and part not in ["発言者情報", "会議録情報"]:
    ...
    def _parse_date(self, date_str: str) -> datetime | None:
        ...
        # 和暦対応
        date_str = date_str.replace("令和", "R").replace("平成", "H").replace("昭和", "S")
